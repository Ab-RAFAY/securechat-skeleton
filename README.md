...
- **Environment**: Clone your fork and run from the repository root. Use a Python 3.10+ virtualenv and install `requirements.txt`.
- **Server (PowerShell)**: Open a terminal in the repo root and run:

```
python .\app\server.py
```

- **Client (PowerShell)**: In a second terminal run:

```
python .\app\client.py
```

- **Expected server output**: On successful start you will see a line similar to:

```
Server listening on 127.0.0.1:9009
```

- **Expected client/server exchanges**: Typical successful run (happy path) will show these events:
   - **Client**: `Server cert verified` after control-plane exchange.
   - **Register**: client sends register payload → server replies `{'type':'register','status':'ok'}` (or `exists`).
   - **Login**: server replies `{'type':'login','status':'ok'}` on valid credentials.
   - **Session**: client and server perform DH and you will see `Session key derived` on client and subsequent `ACK` messages from server on message send.

- **When something fails**: you should see clear JSON error messages from the peer (examples):
   - `{'type':'bad cert','msg': 'EXPIRED'}` or `{'type':'bad cert','msg':'UNTRUSTED_ISSUER'}`
   - `{'type':'sigfail'}` when signature verification fails
   - `{'type':'replay','msg':'seq too small'}` on replayed messages

- **Helpful commands** (Windows PowerShell):

```
# Initialize DB (if using local python/mysql client)
python -m app.storage.db --init
...
- **Deliverables to collect**:
   - `transcripts/<session-file>`: the server & client transcript logs (append-only).
   - `client_receipt.json` / `server_receipt.json` (signed receipts).
   - Wireshark capture files: `evidence_hello.pcapng`, `evidence_register.pcapng`, `evidence_session.pcapng`, `evidence_replay.pcapng`, `evidence_tamper.pcapng`.
   - MySQL dump: `securechat_schema.sql` and a few example user records.

Append these artifacts to your submission (and reference capture filenames in your Test Report).
